...
try:
    ...
    try:
        ...
        # Directory already exists error can be safely ignored
        print("NLTK download directory already exists, continuing...")
    except Exception as e:
        print(f"Warning: Could not download NLTK punkt tokenizer: {str(e)}")
        print("The application will attempt to continue using the simple tokenizer...")
        ...
        if uploaded_files:
            ...
            try:
                if not os.path.exists("data"):
                    ...
                    st.success("Created data directory for storing documents")
            except Exception as e:
                st.error(f"Error creating data directory: {str(e)}")
                st.warning("Will try to continue processing, but file saving may fail")
            ...
            # Report success based on processed files count
            if processed_files > 0:
                st.success(f"Processed {processed_files} out of {len(uploaded_files)} files successfully!")
            else:
                st.error("Failed to process any of the uploaded files. Please check the errors above.")
